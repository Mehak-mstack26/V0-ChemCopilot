...
# # Add proper directory paths based on current structure
...
#             os.path.join(project_root, 'RetroSynthesisAgent', os.path.basename(filename)),
...
#     """
#     # Define folder names with proper paths based on current structure
#     pdf_folder_name = os.path.join(project_root, 'RetroSynthesisAgent', 'pdf_pi')
#     result_folder_name = os.path.join(project_root, 'RetroSynthesisAgent', 'res_pi')
#     result_json_name = 'llm_res'
#     tree_folder_name = os.path.join(project_root, 'RetroSynthesisAgent', 'tree_pi')
...
# # Import necessary modules with try/except to handle potential import errors
# try:
#     # Import name2smiles tool
#     from tools.name2smiles import NameToSMILES
...
#     print("Make sure all required packages are installed and paths are set correctly.")
#     sys.exit(1)
...
#     name_to_smiles_tool = NameToSMILES()
# except Exception as e:
#     print(f"Error initializing NameToSMILES tool: {e}")
#     name_to_smiles_tool = None

# # Monkey patch for file path resolution with timeout protection
# def patched_read_data_from_json(self, filename, timeout=10):
#     """Patched method to read data from JSON with correct path resolution and timeout protection"""
...
#     # Set of paths to check with timeout protection
#     paths_to_check = [
#         actual_path,
#         os.path.join(retrosyn_agent_dir, os.path.basename(filename)),
#         os.path.join(project_root, 'RetroSynthesisAgent', 'RetroSynAgent', os.path.basename(filename)),
...
#     # Check for timeout during the process
#     for path in paths_to_check:
#         if time.time() - start_time > timeout:
#             print(f"Timeout while trying to read {filename}")
#             return {}
...
# # Apply the monkey patch with timeout protection
# try:
#     original_read_data_from_json = CommonSubstanceDB.read_data_from_json
#     CommonSubstanceDB.read_data_from_json = patched_read_data_from_json
# except Exception as e:
#     print(f"Warning: Could not apply monkey patch to CommonSubstanceDB: {e}")

# def get_smiles_for_compound(compound_name):
#     """Get SMILES notation for a compound name using the NameToSMILES tool with timeout protection"""
#     try:
#         if name_to_smiles_tool is None:
#             return None
...
# # Function to run retrosynthesis with parameters and timeout protection
# def retrosyn_main(material=None, num_results=5, alignment=True, expansion=True, filtration=False, timeout=600):
#     """
#     Main function to run the retrosynthesis process with timeout protection.
#     Returns the recommendation text that can be parsed by the UI.
#     """
#     start_time = time.time()
...
#     # Check for existing results first to avoid downloading PDFs if results already exist
#     tree_name_final = os.path.join(tree_folder_name, f'{material}_{"w" if expansion else "wo"}_exp{"_alg" if alignment else ""}.pkl')
    
#     if os.path.exists(tree_name_final):
#         print(f"Found existing tree at {tree_name_final}. Loading directly.")
#         treeloader = TreeLoader()
#         tree_exp = treeloader.load_tree(tree_name_final)
        
#         # Get pathways from existing tree
#         reactions_filtration = ReactionsFiltration()
#         all_pathways_w_reactions = reactions_filtration.getFullReactionPathways(tree_exp)
...
#     # Initialize required objects with timeout protection
#     try:
#         entityalignment = EntityAlignment()
#         treeloader = TreeLoader()
#         tree_expansion = TreeExpansion()
#         reactions_filtration = ReactionsFiltration()
#     except Exception as e:
#         return f"Error initializing required objects: {str(e)}"
    
#     # Check for timeout
#     if time.time() - start_time > timeout:
#         return "Process timed out during initialization"

#     # 1. Query literatures & download with reduced PDF count to speed up
#     try:
#         downloader = PDFDownloader(material, pdf_folder_name=pdf_folder_name, num_results=min(num_results, 5), n_thread=3)
#         pdf_name_list = downloader.main()
#         print(f'Successfully downloaded {len(pdf_name_list)} PDFs for {material}')
        
#         # Check for timeout after download
#         if time.time() - start_time > timeout:
#             return "Process timed out during PDF download"
#     except Exception as e:
#         error_msg = f"Error during PDF download: {str(e)}"
#         print(error_msg)
#         return error_msg
...
#         # Check for timeout after PDF processing
#         if time.time() - start_time > timeout:
#             return "Process timed out during PDF processing"
#     except Exception as e:
...
#         # Check for timeout
#         if time.time() - start_time > timeout:
#             return "Process timed out during root node alignment"
#     except Exception as e:
#         error_msg = f"Error during root node alignment: {str(e)}"
#         print(error_msg)
#         return error_msg
...
#         # Check for timeout
#         if time.time() - start_time > timeout:
#             return "Process timed out during tree construction"
#     except Exception as e:
#         error_msg = f"Error during tree construction: {str(e)}"
#         print(error_msg)
#         return error_msg

#     # Initialize final tree variable
#     tree_exp = tree_wo_exp

#     # 5. Alignment if requested
#     if alignment:
#         try:
#             print('Starting to align the nodes of RetroSynthetic Tree...')
#             tree_name_wo_exp_alg = os.path.join(tree_folder_name, f'{material}_wo_exp_alg.pkl')
...
#             # Update tree_exp for expansion
#             tree_exp = tree_wo_exp_alg
            
#             # Check for timeout
#             if time.time() - start_time > timeout:
#                 return "Process timed out during alignment"
#         except Exception as e:
#             error_msg = f"Error during alignment: {str(e)}"
#             print(error_msg)
#             # Continue with unaligned tree
#             print("Continuing with unaligned tree...")

#     # 6. Tree expansion if requested
#     if expansion:
#         try:
#             results_dict_additional = tree_expansion.treeExpansion(result_folder_name, result_json_name,
#                                                                 results_dict, material, expansion=expansion, max_iter=2)
#             if results_dict_additional:
#                 results_dict = tree_expansion.update_dict(results_dict, results_dict_additional)
...
#             # Check for timeout
#             if time.time() - start_time > timeout:
#                 return "Process timed out during tree expansion"
#         except Exception as e:
#             error_msg = f"Error during tree expansion: {str(e)}"
#             print(error_msg)
#             # Continue with unexpanded tree

#     # 7. Expansion alignment if requested
#     if expansion and alignment:
#         try:
#             tree_name_exp_alg = os.path.join(tree_folder_name, f'{material}_w_exp_alg.pkl')
#             if not os.path.exists(tree_name_exp_alg):
#                 reactions_exp = tree_exp.reactions
...
#             # Check for timeout
#             if time.time() - start_time > timeout:
#                 return "Process timed out during expansion alignment"
#         except Exception as e:
#             error_msg = f"Error during expansion alignment: {str(e)}"
#             print(error_msg)
#             # Continue with unaligned expanded tree

#     # 8. Get full reaction pathways
#     try:
#         all_pathways_w_reactions = reactions_filtration.getFullReactionPathways(tree_exp)
        
#         # Check for timeout
#         if time.time() - start_time > timeout:
#             return "Process timed out when getting reaction pathways"
#     except Exception as e:
#         error_msg = f"Error when getting reaction pathways: {str(e)}"
#         print(error_msg)
#         return error_msg

#     # 9. Filtration if requested
#     if filtration:
#         try:
#             # Filter reactions based on conditions
#             reactions_txt_filtered = reactions_filtration.filterReactions(tree_exp)
...
#             # Check for timeout
#             if time.time() - start_time > timeout:
#                 return "Process timed out during filtration"
#         except Exception as e:
#             error_msg = f"Error during filtration: {str(e)}"
#             print(error_msg)
#             # Continue with unfiltered pathways
...
#         # Check for timeout
#         if time.time() - start_time > timeout:
#             return "Process timed out during recommendation"
...
# def recommendReactions(prompt, result_folder_name, response_name, timeout=60):
#     start_time = time.time()
#     try:
#         res = GPTAPI().answer_wo_vision(prompt)
...
#         return recommend_reactions_txt
#     except Exception as e:
#         error_msg = f"Error in recommendReactions: {str(e)}"
#         print(error_msg)
#         return error_msg
...
#     def _run(self, compound_name: str, num_results: int = 5, timeout: int = 600) -> dict:
#         """Run retrosynthesis analysis on the given compound with timeout protection"""
#         try:
#             # Start a timer
#             start_time = time.time()
#             print(f"Starting retrosynthesis for {compound_name} with timeout of {timeout} seconds")
            
#             # Check if we already have cached results for this compound
#             cached_result_path = os.path.join(
#                 project_root, 'RetroSynthesisAgent', 'res_pi', f'recommend_pathway1_{compound_name}.txt'
#             )
#             if os.path.exists(cached_result_path):
#                 print(f"Using cached results for {compound_name}")
#                 with open(cached_result_path, 'r') as f:
#                     raw_output = f.read()
#             else:
#                 # Call the main function with parameters and timeout protection
#                 raw_output = retrosyn_main(
#                     material=compound_name,
#                     num_results=num_results,
#                     alignment=True,
#                     expansion=True,
...
#             if raw_output.startswith("Error") or raw_output.startswith("Process timed out"):
#                 return {
#                     "status": "error",
#                     "message": raw_output
#                 }
...
#                 return {
#                     "status": "error",
#                     "message": f"Error parsing reaction data: {str(e)}",
...
#                 for reaction in reactions:
#                     futures.append(executor.submit(process_reaction_smiles, reaction))
                    
#                 # Wait for all to complete with timeout
#                 remaining_time = max(10, timeout - (time.time() - start_time))
...
# def process_reaction_smiles(reaction):
#     """Process a single reaction to ensure proper SMILES with timeout protection"""
#     try:
#         # Clean up the reaction SMILES - remove placeholders if present
#         original_smiles = reaction["reaction_smiles"]
#         if original_smiles:
...
#         reaction["cleaned_reaction_smiles"] = None
    
#     return reaction

# # Function to use for direct calling (without using as a LangChain tool)
# def run_retrosynthesis(compound_name, num_results=5, timeout=600):
#     """
#     Run the RetroSynthesis Agent for a given compound and process the results with timeout protection
#     """
#     tool = RetroSynthesisTool()
#     return tool._run(compound_name, num_results, timeout)
...
def retrosyn_main(material=None, num_results=10, alignment=True, expansion=True, filtration=False):
    ...
    if not all_pathways_w_reactions:
        ...
        try:
            ...
            # Ensure pathways are formatted correctly for the prompt
            # Convert each element (assuming they are ReactionPathway objects or similar) to string
            pathways_str_list = []
            for path_obj in all_pathways_w_reactions:
                 try:
                      pathways_str_list.append(str(path_obj))
            ...
            pathways_str = "\n\n".join(pathways_str_list)
            # Limit length for logging if necessary
            # print(f"[DEBUG][retrosyn_main] Pathways string for prompt (sample):\n---\n{pathways_str[:1000]}...\n---")

            prompt_recommend1 = prompts.recommend_prompt_commercial.format(all_pathways = pathways_str)
            # Limit length for logging if necessary
            # print(f"[DEBUG][retrosyn_main] Recommendation prompt (sample):\n---\n{prompt_recommend1[:1000]}...\n---")
            ...
            recommend1_reactions_txt = f"Error during recommendation generation: {str(e)}"
